...
describe("resolveThreadDeepLinkScheme", () => {
  it("derives the scheme from the product branding", () => {
    assert.equal(
      DesktopDeepLink.resolveThreadDeepLinkScheme({ baseName: "T3 Code", isDevelopment: false }),
      "t3code",
    );
    assert.equal(
  ...
  it("suffixes development builds so they do not claim the installed scheme", () => {
    assert.equal(
      DesktopDeepLink.resolveThreadDeepLinkScheme({ baseName: "T3 Code", isDevelopment: true }),
      "t3code-dev",
    );
    assert.equal(
...
interface TestHarness {
  ...
  readonly registeredSchemes: string[];
}
...
const makeServices = (harness: TestHarness, overrides: TestEnvironmentOverrides = {}) => {
  const environment = DesktopEnvironment.DesktopEnvironment.of({
    isDevelopment: overrides.isDevelopment ?? false,
    isPackaged: overrides.isPackaged ?? false,
    platform: overrides.platform ?? "darwin",
    branding: { baseName: overrides.baseName ?? "T3 Code" },
  ...
  const electronApp = {
    ...
    setAsDefaultProtocolClient: (protocol: string) =>
      Effect.sync(() => {
        harness.registeredSchemes.push(protocol);
        return true;
      }),
...
describe("DesktopDeepLink", () => {
  ...
  it.effect("claims the branded scheme on packaged macOS and Windows builds", () => {
    const harness = makeHarness();
    const renderer = makeSender(7);
    ...
    return Effect.gen(function* () {
      ...
      // The branded scheme is also the one parsed; a t3code:// link is not ours.
      assert.deepEqual(yield* subscribeAs(harness, renderer), PAYLOAD);
    });
  });

  it.effect("does not claim the scheme for unpackaged or Linux builds", () => {
    const dev = makeHarness();
    const linux = makeHarness();

    return Effect.gen(function* () {
      yield* configureWith(makeServices(dev, { isPackaged: false, platform: "win32" }));
      yield* configureWith(makeServices(linux, { isPackaged: true, platform: "linux" }));

      assert.deepEqual(dev.registeredSchemes, []);
      assert.deepEqual(linux.registeredSchemes, []);
    });
  });

  it.effect("ignores links carrying the sibling product's scheme", () => {
    const harness = makeHarness();
    const renderer = makeSender(7);

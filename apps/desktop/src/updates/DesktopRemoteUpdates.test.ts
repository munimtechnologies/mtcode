...
describe("DesktopRemoteUpdates", () => {
  it.effect("drives check, download, and install with no local confirmation", () => {
    ...
    return runRemoteUpdatesTest(harness, ({ reports, requests, commits }) =>
      Effect.gen(function* () {
        ...
        // Personal fork: update-downloaded auto-installs; the commit is not
        // what starts the installer.
        ...
        yield* settle;
        // The commit joins the auto-install's shutdown: no second installer
        // and no "failed" marker.
        assert.equal(harness.quitAndInstalls(), 1);
        assert.equal(terminalReports(reports).length, 1);
  ...
  it.effect("does not join a normal app quit as an update install", () => {
    // Personal fork: update-downloaded auto-installs, so fail that install to
    // reach "downloaded, app still running" before the normal quit.
    const harness = makeHarness({
      quitAndInstall: Effect.fail(
        new ElectronUpdater.ElectronUpdaterQuitAndInstallError({
          channel: "latest",
  ...
  it.effect("does not misread a lingering install error as a failed retry", () => {
    ...
    return runRemoteUpdatesTest(harness, ({ reports, requests, commits }) =>
      Effect.gen(function* () {
        // Personal fork: the failed first attempt is the auto-install that
        // update-downloaded triggers, outside any remote request.
        harness.emit("update-available", { version: "1.2.4" });
        yield* settle;
        harness.emit("update-downloaded", { version: "1.2.4" });
        yield* settle;
        ...
        yield* settle;
        assert.equal(harness.quitAndInstalls(), 1);
        // First attempt failed; state is "downloaded" with a lingering
  ...
  it.effect("retries a download refused while the check still holds the reservation", () => {
    ...
    return runRemoteUpdatesTest(harness, ({ reports, requests }) =>
      Effect.gen(function* () {
        ...
        // Personal fork: update-downloaded auto-installs.
        assert.equal(harness.quitAndInstalls(), 1);
      }),
    );
  });
  ...
  it.effect("waits for the download reservation before reporting prepared", () => {
    ...
    return runRemoteUpdatesTest(harness, ({ reports, requests }) =>
      Effect.gen(function* () {
        ...
        // Personal fork: the auto-install waited for the same reservation and
        // then ran; the prepared report joins it.
        assert.equal(harness.quitAndInstalls(), 1);
      }),
    );
  });
  ...
  it.effect("joins an install that is already tearing the app down", () => {
    ...
    return runRemoteUpdatesTest(harness, ({ reports, requests }) =>
      Effect.gen(function* () {
        ...
        harness.emit("update-downloaded", { version: "1.2.4" });
        yield* settle;
        // Personal fork: the auto-install already owns the shutdown and keeps
        // its reservation until the app exits.
        assert.equal(harness.quitAndInstalls(), 1);
        assert.isTrue(yield* Ref.get(desktopState.quitting));
  ...
  it.effect("waits for a background check before installing a prepared update", () => {
    ...
    const harness = makeHarness({
      ...
      // Personal fork: fail the auto-install that update-downloaded triggers so
      // the app is still running when the background check starts.
      quitAndInstall: Effect.suspend(() => {
        installAttempts += 1;
        return installAttempts === 1
          ? Effect.fail(
  ...
  it.effect("fails a prepared install when its background check stays blocked", () => {
    ...
    const harness = makeHarness({
      ...
      // Personal fork: every update-downloaded auto-installs; keep those
      // attempts failing so the app stays running throughout.
      quitAndInstall: Effect.fail(
        new ElectronUpdater.ElectronUpdaterQuitAndInstallError({
          channel: "latest",
          isSilent: true,
    ...
    return runRemoteUpdatesTest(harness, ({ reports, requests, commits }) =>
      Effect.gen(function* () {
        ...
        yield* Fiber.join(backgroundCheck);
        yield* settle;
        // Only the auto-install attempted again; the failed commit did not.
        assert.equal(harness.quitAndInstalls(), 2);
  ...
  it.effect("does not install after cancellation wins the commit claim", () => {
    ...
    return runRemoteUpdatesTest(harness, ({ reports, requests, commits, cancellations }) =>
      Effect.gen(function* () {
        ...
        yield* settle;
        // Personal fork: update-downloaded already auto-installed once.
        assert.equal(harness.quitAndInstalls(), 1);
        yield* Queue.offer(commits, {
          version: 1,
          type: "commitDesktopUpdate",

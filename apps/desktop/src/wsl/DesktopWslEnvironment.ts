...
const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_BINARY_MISSING_EXIT_CODE
    ? `WSL support is missing from this ${desktopAppDisplayName()} build: the packaged Linux node-pty binary was not included. Install a build that includes WSL support.`
    : null;

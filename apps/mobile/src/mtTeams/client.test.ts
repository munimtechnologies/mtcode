...
// The baked service origin comes from the expo config extra (app.config.ts
// bakes `extra.mtTeams.url`); the client must read it from there and never
// from storage.
vi.mock("expo-constants", () => ({
  default: {
    expoConfig: {
      extra: { mtTeams: { url: "https://acme.convex.site" } },
    },
  },
}));
...
describe("defaultServiceUrl", () => {
  it("reads the baked origin from the expo config extra", () => {
    expect(defaultServiceUrl()).toBe("https://acme.convex.site");
  });
});
...
describe("stored session", () => {
  it("persists only the session (never a URL) and rehydrates the baked service URL", async () => {
    ...
    expect(secureStore.get(MT_TEAMS_STORAGE_KEY)).not.toContain("convex.site");
  ...
  it("returns null for malformed or missing records instead of throwing", async () => {
    expect(parseStoredSessionValue("not json")).toBeNull();
    expect(parseStoredSessionValue(JSON.stringify({ sessionToken: 5 }))).toBeNull();
    secureStore.set(MT_TEAMS_STORAGE_KEY, "not json");
    await expect(loadStoredSession()).resolves.toBeNull();
  });
});
...
describe("endpoints", () => {
  ...
  it("revokes, accepts, declines, leaves, and removes through the documented paths", async () => {
    const cases: ReadonlyArray<{
      ...
      {
        run: () => sendMessage(session, { sharedThreadId: "shared-1", text: "On it." }),
        path: "/api/messages/send",
        body: { sharedThreadId: "shared-1", text: "On it." },
      },
    ];
    for (const testCase of cases) {
      const fetchMock = stubFetch({ ok: true, teamId: "team-1", name: "Relay", messageId: "m-1" });
      await testCase.run();

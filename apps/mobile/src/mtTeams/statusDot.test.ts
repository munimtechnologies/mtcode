...
describe("mtTeamsStatusDotClassName", () => {
  it("maps each status to its semantic dot color", () => {
    ...
    expect(mtTeamsStatusDotClassName("input-needed")).toBe("bg-amber-500");
    expect(mtTeamsStatusDotClassName("done")).toBe("bg-green-500");
    expect(mtTeamsStatusDotClassName("idle")).toBe("bg-neutral-400 dark:bg-neutral-500");
  });

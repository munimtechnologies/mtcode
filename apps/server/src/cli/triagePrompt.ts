...
export const TRIAGE_PLAYBOOK = `# MT Code triage playbook

You are a support engineer for MT Code (https://github.com/munimtechnologies/mtcode), working
...
Fetch https://raw.githubusercontent.com/munimtechnologies/mtcode/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/munimtechnologies/mtcode <source-cache-dir>/<hash>
...
- How is MT Code running on this machine: \`npx t3 serve\` in a terminal, the
...
Search existing issues in munimtechnologies/mtcode (use \`gh\`, or the public GitHub search
...
Do not patch the MT Code source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/munimtechnologies/mtcode/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your MT Code triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A MT Code user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# MT Code triage context
...
- Repo: https://github.com/munimtechnologies/mtcode

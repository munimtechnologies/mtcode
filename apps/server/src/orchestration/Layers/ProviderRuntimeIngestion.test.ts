...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("badges a stopped turn by how it ended, not by a spent bucket it never ran in", async () => {
    const harness = await createHarness();
    const provider = ProviderDriverKind.make("claudeAgent");
    const usageBilling = (
      thread: { readonly activities: ReadonlyArray<unknown> },
      turnId: string,
    ) => {
      const activity = (thread.activities as ReadonlyArray<ProviderRuntimeTestActivity>).find(
        (entry) => entry.kind === "provider.turn.usage" && entry.turnId === turnId,
      );
    ...
    // The Fable-only weekly bucket is spent; the account's own windows are not.
    await harness.emitAndDrain([
      {
        type: "account.rate-limits.updated",
        eventId: asEventId("evt-limits"),
        provider,
        threadId: asThreadId("thread-1"),
        createdAt: "2026-01-01T00:00:00.000Z",
        payload: {
          limits: {
            windows: [
              { id: "five_hour", kind: "session", label: "Session", usedPercent: 41 },
              { id: "seven_day", kind: "weekly", label: "Weekly", usedPercent: 64 },
              {
                id: "seven_day_fable",
                kind: "weekly",
                label: "Weekly · Fable",
      ...
      {
        type: "turn.started",
        eventId: asEventId("evt-stop-started"),
        provider,
        threadId: asThreadId("thread-1"),
        createdAt: "2026-01-01T00:00:01.000Z",
        turnId: asTurnId("turn-stopped"),
      },
      {
        type: "turn.completed",
        eventId: asEventId("evt-stop-completed"),
        provider,
        threadId: asThreadId("thread-1"),
    ...
    let thread = await waitForThread(
      harness.readModel,
      (entry) => usageBilling(entry, "turn-stopped") !== undefined,
    );
    expect(usageBilling(thread, "turn-stopped")).toMatchObject({
      billing: "included",
      windowLabel: "Weekly",
    ...
    // A turn the limit genuinely stopped still reads as limited.
    await harness.emitAndDrain([
      {
        type: "turn.started",
        eventId: asEventId("evt-limited-started"),
        provider,
        threadId: asThreadId("thread-1"),
      ...
      {
        type: "turn.completed",
        eventId: asEventId("evt-limited-completed"),
        provider,
        threadId: asThreadId("thread-1"),
    ...
    thread = await waitForThread(
      harness.readModel,
      (entry) => usageBilling(entry, "turn-limited") !== undefined,
    );
    expect(usageBilling(thread, "turn-limited")).toMatchObject({ billing: "limited" });

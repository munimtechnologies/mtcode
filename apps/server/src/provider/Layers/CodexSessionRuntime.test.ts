...
describe("T3 tool instructions", () => {
  ...
  it("omits the tool entry entirely when no tools are attached", () => {
    ...
    NodeAssert.deepStrictEqual(Object.keys(context), ["t3_code_runtime", "t3_code_chat_history"]);
  });
  ...
  it("describes Computer Use pointer tools only when the desktop MCP is attached", () => {
    ...
    NodeAssert.doesNotMatch(
      buildCodexAdditionalContext(runtime, false).t3_code_tools?.value ?? "",
...
describe("hasConfiguredMcpServer", () => {
  ...
  it("matches a named MCP server without treating a sibling as present", () => {
    const args = [
      "-c",
      `mcp_servers.${DESKTOP_MCP_SERVER_NAME}.command='/usr/bin/munim-computer-use'`,
    ];

    ...
    withThreadLock(
      ...
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          // Do not gate on the HTTP t3-code MCP session (fresh Devin threads).
          // Devin launches stdio MCP servers itself, so the desktop MCP goes in
          // as a plain stdio entry alongside the bridged t3-code endpoint.
          const desktopMcp = yield* resolveDesktopMcp();
          const desktopMcpEntries: ReadonlyArray<EffectAcpSchema.McpServer> = desktopMcp
            ? [
                {
          ...
          const runtime = yield* options.makeRuntime({
            ...
            ...(mcpSession || desktopMcpEntries.length > 0
              ? {
                  mcpServers: (
                    capabilities: EffectAcpSchema.McpCapabilities | undefined,
                  ): ReadonlyArray<EffectAcpSchema.McpServer> => {
                    if (!mcpSession) return desktopMcpEntries;
                    const httpEntry = {
                      type: "http" as const,
                      name: "t3-code",
                    ...
                    if (capabilities?.http === true || mcpBridgePath === undefined) {
                      return [httpEntry, ...desktopMcpEntries];
                    }
                    return [
                      mcpStdioBridgeEntry({

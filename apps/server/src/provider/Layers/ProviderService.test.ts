...
describe("agent browser access", () => {
  ...
  it.effect("issues a credential without preview when agent browser access is off", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [{ threadId, capabilities: ["pull-requests", "thread-metadata", "thread-reference", "worktree"] }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("issues a credential with preview when agent browser access is on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["device", "preview", "pull-requests", "thread-metadata", "thread-reference", "worktree"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("drops only the preview capability when browser access alone is off", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["device", "pull-requests", "thread-metadata", "thread-reference", "worktree"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("requests an MCP credential when the project overrides browser access to on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["preview", "pull-requests", "thread-metadata", "thread-reference", "worktree"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );

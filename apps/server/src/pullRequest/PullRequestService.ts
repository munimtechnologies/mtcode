...
export const make = Effect.gen(function* () {
            ...
            Effect.flatMap((upstream) =>
              upstream !== null && upstream.toLowerCase() === wanted
                ? Effect.succeed({
                    ...own,
                    repository: upstream,
                    // Keyed records (viewed files) follow the repository read, not the fork's.
                    remote: normalizeGitRemoteUrl(`https://${own.host}/${upstream}`),
                    isUpstream: true,
                  })
                : Effect.fail(
                    new PullRequestOperationError({
      ...
      Effect.map((upstreams) => {
        ...
        for (const [index, upstream] of upstreams.entries()) {
          ...
          added.push({
            ...
            repository: upstream,
            // Keyed records (viewed files) follow the repository read, not the fork's.
            remote: normalizeGitRemoteUrl(`https://${project.host}/${upstream}`),
            isUpstream: true,
          });
        }
  ...
  const refEpoch = (ref: PullRequestRef) =>
    Math.max(projectEpochs.get(ref.projectId) ?? projectEpochFloor, refEpochs.get(ref));
  ...
  const filesViewedEpochs = makeScopedEpochs();
  const filesViewedEpoch = (ref: PullRequestRef) => filesViewedEpochs.get(ref);
  const bumpFilesViewedEpoch = (ref: PullRequestRef) => filesViewedEpochs.bump(ref);
  ...
  const diffCache = yield* Cache.makeWith(
    (key: string) => {
      const [reference, , cursor, commit] = JSON.parse(key) as [
        string,
        number,
        string | null,
        string | null,
    ...
    },
    {
      ...
      timeToLive: (exit, key) => {
        ...
        const commit = (JSON.parse(key) as ReadonlyArray<unknown>)[3];
  ...
  const invalidate: PullRequestService["Service"]["invalidate"] = Effect.fn(
    ...
    if (reference !== undefined) {
      yield* canonicalRef(reference).pipe(
        Effect.flatMap((ref) =>
          readCache.invalidate(refScope(ref)).pipe(
            Effect.andThen(
              Effect.sync(() => {
                if (input.resource === "diff") {
                  bumpAggregateDiffEpoch(ref);
                  return;
                }
                bumpRefEpoch(ref);
              }),

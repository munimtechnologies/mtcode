  ...
  const cloneRepository = Effect.fn("SourceControlRepositoryService.cloneRepository")(function* (
    ...
    yield* git
      .execute({
        ...
        // SSH remotes may ask for a key passphrase or a password; the prompt
        // is relayed to the client instead of hanging on a closed stdin.
        ...(isSshRemoteUrl(prepared.cloneUrl)
          ? { sshAuthentication: { destination: prepared.cloneUrl } }
          : {}),
      })
      .pipe(
        Effect.mapError(

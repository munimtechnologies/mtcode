...
const insensitiveLiteral = { caseSensitive: false, regex: false };
...
describe("findTerminalSearchMatches", () => {
  ...
  it("respects case-sensitive option", () => {
    ...
    expect(findTerminalSearchMatches(rows, "hello", { caseSensitive: true, regex: false })).toEqual(
      { matches: [match(0, 12, 17)], truncated: false, error: null },
    );
  });
  it("handles regex patterns", () => {
    const rows = { texts: ["foo123bar456"], wraps: [false] };
    expect(
      findTerminalSearchMatches(rows, "(\\d+)", { caseSensitive: false, regex: true }),
    ).toEqual({ matches: [match(0, 3, 6), match(0, 9, 12)], truncated: false, error: null });
  });
  it("returns error for invalid regex", () => {
    const rows = { texts: ["test"], wraps: [false] };
    expect(
      findTerminalSearchMatches(rows, "[invalid(", { caseSensitive: false, regex: true }),
    ).toEqual({ matches: [], truncated: false, error: "Invalid regular expression" });
  });
  it("skips zero-length matches", () => {
    const rows = { texts: ["aaa"], wraps: [false] };
    expect(findTerminalSearchMatches(rows, "a*", { caseSensitive: false, regex: true })).toEqual({
      matches: [match(0, 0, 3)],
      truncated: false,
      error: null,
    });
  ...
  it("truncates only when matches exceed the maximum", () => {
    ...
    expect([exactResult.matches.length, exactResult.truncated, exactResult.error]).toEqual([
      MAX_TERMINAL_SEARCH_MATCHES,
      false,
      null,
    ]);
    ...
    expect([overflowResult.matches.length, overflowResult.truncated, overflowResult.error]).toEqual(
      [MAX_TERMINAL_SEARCH_MATCHES, true, null],
    );
  });
});

...
```text
Claude usage limit reached. This turn is paused until the 5-hour limit resets at Feb 3, 3:00 PM PST.
```

Claude Code holds the turn until that window reopens, so the thread keeps showing as working. You
can wait for the reset, or stop the turn and pick the work back up later.

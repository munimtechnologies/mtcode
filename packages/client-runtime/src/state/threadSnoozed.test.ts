...
describe("threadUsageLimitResetsAt", () => {
  const RESETS_AT = "2026-04-10T14:00:00.000Z";
  function shell(session: Partial<NonNullable<ThreadSnoozeShell["session"]>> | null) {
    return {
      session:
        session === null
          ? null
  ...
  it("reads the reset off a session parked on a usage-limit failure", () => {
    expect(
      threadUsageLimitResetsAt(
        shell({ lastErrorKind: "usage_limit", lastErrorResetsAt: RESETS_AT }),
      ),
    ).toBe(RESETS_AT);
  });

  it("is null for other failures, a recovered session, an unclassified reset, or no session", () => {
    expect(threadUsageLimitResetsAt(shell({ lastErrorResetsAt: RESETS_AT }))).toBeNull();
    expect(
      threadUsageLimitResetsAt(
        shell({ status: "ready", lastErrorKind: "usage_limit", lastErrorResetsAt: RESETS_AT }),
      ),
    ).toBeNull();

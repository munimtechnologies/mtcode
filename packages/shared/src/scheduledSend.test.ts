...
describe("nextScheduledSendOccurrence", () => {
  ...
  it("keeps the wall-clock time across a DST change", () => {
    ...
    expect(
      nextScheduledSendOccurrence(
        ...
        rule("daily"),
        Date.parse("2026-10-31T13:00:01.000Z"),
      ),
    ).toBe("2026-11-01T14:00:00.000Z");
  });

  it("advances daily, weekdays, weekly, and monthly", () => {
    expect(nextScheduledSendOccurrence(previous, rule("daily"), now)).toBe(
    ...
    expect(
      nextScheduledSendOccurrence(
        ...
        rule("weekdays"),
        Date.parse("2026-09-18T13:00:01.000Z"),
      ),
    ).toBe("2026-09-21T13:00:00.000Z");
    expect(nextScheduledSendOccurrence(previous, rule("weekly"), now)).toBe(
      "2026-09-23T13:00:00.000Z",
    );
    expect(nextScheduledSendOccurrence(previous, rule("monthly"), now)).toBe(
  ...
  it("skips occurrences that already passed while the server was down", () => {
    ...
    expect(nextScheduledSendOccurrence(previous, rule("daily"), later)).toBe(
